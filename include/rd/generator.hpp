...
template<typename Allocator>
concept proto_allocator = std::same_as<Allocator, void> ||(
  std::is_nothrow_copy_constructible_v<Allocator> &&requires(Allocator) {
...
template<typename R> struct elements_of {
  R &&__range;// \expos
  ...
  constexpr R &&get() &&noexcept { return std::forward<R>(__range); }
};

template<typename R> elements_of(R &&r) -> elements_of<decltype(r)>;
...
template<proto_allocator Alloc> class promise_base_alloc;

template<> class promise_base_alloc<void> {
  ...
  static constexpr std::size_t offset_of_allocator(std::size_t frameSize) {
    return aligned_allocation_size(
      offset_of_deleter(frameSize) + sizeof(deleter), alignof(Alloc));
  }
...
public:
  static void *operator new(std::size_t size) {
    return promise_base_alloc::operator new(
      size, std::allocator_arg, std::allocator<std::byte>());
  }


  template<typename Alloc, typename... Args>
  static void *operator new(std::size_t frameSize,
    std::allocator_arg_t,
    Alloc alloc,
    Args &...) {
    ...
    deleter f = [](void *ptr, std::size_t frameSize) {
      if constexpr (allocator_needs_to_be_stored<Alloc>) {
        ...
        alloc.~Alloc();
        localAlloc.deallocate(static_cast<std::byte *>(ptr),
          padded_frame_size<bytes_alloc>(frameSize));
      } else {
        bytes_alloc alloc;
        alloc.deallocate(static_cast<std::byte *>(ptr),
          padded_frame_size<bytes_alloc>(frameSize));
      }
    };
    ::new (static_cast<void *>(std::addressof(get_deleter(frame, frameSize))))
      deleter(std::move(f));
    if constexpr (allocator_needs_to_be_stored<bytes_alloc>) {
      // Store allocator at end of the coroutine frame.
      // Assuming the allocator's move constructor is non-throwing (a
      // requirement for allocators)
      ::new (static_cast<void *>(
        std::addressof(get_allocator<bytes_alloc>(frame, frameSize))))
        bytes_alloc(std::move(alloc));
    }
    return frame;
  }
  ...
  static void *operator new(std::size_t frameSize,
    ...
    Args &...) {
    return promise_base_alloc::operator new(
      frameSize, std::allocator_arg, std::move(alloc));
  }
...
public:
  ...
  static void *operator new(std::size_t frameSize,
    ...
    // for allocators)
    ::new (static_cast<void *>(std::addressof(get_allocator(frame, frameSize))))
      Alloc(std::move(balloc));
  ...
  static void operator delete(void *ptr, std::size_t frameSize) {
    ...
    bytes_alloc localAlloc(std::move(alloc));
    alloc.~Alloc();
    localAlloc.deallocate(
      static_cast<char *>(ptr), padded_frame_size(frameSize));
  }
};
...
template<typename Ref,
  ...
  proto_allocator Alloc = void>
class generator;


template<typename Ref> struct promise_base {
  ...
  template<typename Gen> struct yield_sequence_awaiter {
...
template<typename Ref, typename Value, proto_allocator Alloc> class generator {
...
constexpr bool std::ranges::enable_view<rd::generator<T, U>> = true;
}
